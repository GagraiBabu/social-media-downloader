...
async def get_video_info_endpoint(payload: InfoRequest):
    ...
    info = extract_media_info(norm_url, detected_platform=platform)
...
async def download_video_endpoint(payload: DownloadRequest, background_tasks: BackgroundTasks):
    ...
    filepath, clean_filename, temp_dir = download_media_file(
        url=norm_url,
        requested_quality=payload.quality or "1080p",
        detected_platform=platform,
    )
