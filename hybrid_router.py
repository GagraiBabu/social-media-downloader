...
def _download_from_worker(worker_url: str, media_url: str, quality: str, platform: Optional[str]):
    ...
    try:
        ...
        if ext and ext != ".mp4":
            final_target = os.path.join(temp_dir, filename)
...
def download_media_file_hybrid(url: str, requested_quality: Optional[str] = None, detected_platform: Optional[str] = None):
    ...
    # Layer 1: existing Render/direct path only.
    try:
        return download_media_file(url, quality, detected_platform, allow_residential_fallback=False)
    except Exception as exc:
        last_error = exc

    # Layer 2: Google Cloud Run.
    ...
    # Layer 4: existing Webshare Residential fallback.
    try:
