...
app = FastAPI(title="Social Media Downloader Hybrid Worker", version=settings.APP_VERSION, lifespan=lifespan)
...
async def internal_download(payload: WorkerDownloadRequest, authorization: str | None = Header(default=None)):
    ...
    filepath, filename, _temp_dir = await run_in_threadpool(
        download_media_file, normalized_url, payload.quality, platform, False
    )
